...
export async function GET(req: Request) {
  try {
    ...
    // Trouver toutes les règles actives avec déclencheur "quotidien"
    const { data: rules, error } = await supabaseAdmin
      .from('regles')
      .select('user_id, id')
      .eq('declencheur', 'quotidien')
      .eq('actif', true);

    if (error) throw error;

    if (!rules || rules.length === 0) {
      return NextResponse.json({ message: 'Aucune règle quotidienne à exécuter' }, { status: 200 });
    }
    ...
    const promises = rules.map(rule => 
      processPayoutsForUser(rule.user_id, 0, rule.id, true)
        .catch(err => console.error(`Erreur CRON Quotidien pour user ${rule.user_id}:`, err))
    );
